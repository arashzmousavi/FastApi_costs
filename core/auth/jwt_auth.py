...
def get_auth_username(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Authentication failed, decode faild.",
        )
...
def decode_refresh_token(token):
    try:
        ...
        if not user_id:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="user_id not in the payload.",
            )

        if decode.get("type") != "refresh":
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="token type not valid.",
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="decode faild."
        )
    except Exception as e:
        raise HTTPException(
